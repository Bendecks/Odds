...
class ModelValidationReadinessTests(unittest.TestCase):
    def test_empty_history_is_not_promotion_ready(self):
        r=build([],[]);self.assertFalse(r['promotion_ready']);self.assertEqual(r['settled_decisive_bets'],0);self.assertIsNone(r['roi_pct'])
    ...
    def test_computes_roi_clv_and_brier_from_new_model_settlements(self):
        ...
        settled=[{'signal_key':'A|h2h|A|t|v3','result':'win','stake_dkk':1,'odds':2.0,'closing_odds':2.0,'fair_probability':0.6}]
        r=build(signals,settled)
        self.assertEqual(r['roi_pct'],100.0);self.assertEqual(r['mean_clv_pct'],0.0);self.assertAlmostEqual(r['brier_score'],0.16);self.assertFalse(r['promotion_ready'])
