...
from model_validation_readiness import build, roi_ci

class ModelValidationReadinessTests(unittest.TestCase):
    def test_empty_history_is_not_promotion_ready(self):
        r=build([],[]);self.assertFalse(r['promotion_ready']);self.assertEqual(r['settled_decisive_bets'],0);self.assertIsNone(r['roi_pct']);self.assertIsNone(r['roi_bootstrap_95_pct'])
    ...
    def test_computes_roi_clv_and_brier_from_new_model_settlements(self):
        ...
        self.assertEqual(r['roi_pct'],100.0);self.assertEqual(r['mean_clv_pct'],0.0);self.assertAlmostEqual(r['brier_score'],0.16);self.assertFalse(r['promotion_ready']);self.assertEqual(r['current_model_version'],'market-consensus-v3')
