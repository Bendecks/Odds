...
CAND=pathlib.Path('data/value_candidates.json'); OBS=pathlib.Path('data/bet365_observations.jsonl'); SUMMARY=pathlib.Path('output/bet365_market_summary.json'); STATUS=pathlib.Path('output/bet365_join_status.json'); DIAG=pathlib.Path('output/reference_match_diagnostics.json'); UNIBET=pathlib.Path('output/unibet_observer.json')
...
def bookmaker_markets(data,name):
    books=(data or {}).get('bookmakers') or {}
    wanted=norm(name)
    for key,value in books.items():
        if norm(key)==wanted:return value if isinstance(value,list) else []
    return []
def bet365_markets(data):return bookmaker_markets(data,'Bet365')
def market_rows(event,markets,now,bookmaker='Bet365'):
    rows=[]
    for m in markets:
        ...
        for line in m.get('odds') or []:
            ...
            for field,value in line.items():
                ...
                if price<=1:continue
                rows.append({'event_id':event.get('id'),'event':f"{event.get('home')} vs {event.get('away')}",'home':event.get('home'),'away':event.get('away'),'sport':event.get('sport'),'league':event.get('league'),'commence_time':event.get('date') or event.get('startTime') or event.get('commence_time'),'market':name,'selection':field,'raw_selection_id':raw_id,'raw_selection_name':raw_name,'raw_selection_label':raw_label,'odds':price,'line':line_value,'timestamp':updated,'bookmaker':bookmaker,'source':'odds-api.io'})
    return rows
MARKET_ALIASES={'h2h':('ml','moneyline','h2h','1x2'),'totals':('totals','goals over/under','alternative total goals','alternative goal line'),'spreads':('spread','alternative asian handicap'),'btts':('both teams to score','teams to score')}
def market_matches(candidate_market, bet365_market):
    wanted=MARKET_ALIASES.get(str(candidate_market or '').lower(),(str(candidate_market or '').lower(),));actual=str(bet365_market or '').lower();return actual in wanted
...
def bet365_field(candidate,event):
    ...
    if market=='h2h':
        home=norm(event.get('home'));away=norm(event.get('away'));return 'home' if pick==home else 'away' if pick==away else 'draw' if pick in ('draw','uafgjort') else None
    if market=='totals': return 'over' if pick=='over' else 'under' if pick=='under' else None
    if market=='spreads':
        home=norm(event.get('home'));away=norm(event.get('away'));return 'home' if pick==home else 'away' if pick==away else None
    if market=='btts':
...
def fetch_odds(prioritized,now):
    cache={};observations=[];unibet=[];errors=[];attempts=successes=batch_attempts=batch_successes=fallback_attempts=fallback_successes=0;event_by_id={str(e['id']):e for e in prioritized}
    for group in chunks(prioritized,BATCH_SIZE):
        ids=[str(e['id']) for e in group];attempts+=1;batch_attempts+=1
        try:
            payload=get('/odds/multi',{'eventIds':','.join(ids),'bookmakers':BOOKMAKERS});successes+=1;batch_successes+=1
            rows=payload if isinstance(payload,list) else payload.get('data') or payload.get('events') or [] if isinstance(payload,dict) else [];returned=set()
            for data in rows:
                eid=str((data or {}).get('id') or (data or {}).get('eventId') or '')
                if eid not in event_by_id:continue
                returned.add(eid);cache[eid]=data;observations.extend(market_rows(event_by_id[eid],bet365_markets(data),now));unibet.extend(market_rows(event_by_id[eid],bookmaker_markets(data,'Unibet'),now,'Unibet'))
        ...
        for eid in missing:
            ...
            try:
                data=get('/odds',{'eventId':eid,'bookmakers':BOOKMAKERS});successes+=1;fallback_successes+=1;cache[eid]=data;observations.extend(market_rows(event_by_id[eid],bet365_markets(data),now));unibet.extend(market_rows(event_by_id[eid],bookmaker_markets(data,'Unibet'),now,'Unibet'))
            except requests.RequestException as exc:
    ...
    return cache,observations,unibet,errors,attempts,successes,batch_attempts,batch_successes,fallback_attempts,fallback_successes
def observer_summary(rows,now):
    markets=collections.Counter(x['market'] for x in rows);events=set(str(x.get('event_id')) for x in rows);selections=collections.Counter(x['selection'] for x in rows)
    return {'generated_at':now.isoformat(),'mode':'observer_only','bookmaker':'Unibet','events_with_observations':len(events),'observations':len(rows),'unique_markets':len(markets),'top_markets':markets.most_common(100),'selection_fields':selections.most_common(50),'note':'Diagnostic only. Unibet observations do not affect fair probabilities, reference depth, Bet365 verification, PAPER qualification, or PLAY.'}
def unavailable(refs,now,exc):
    clear_bet365(refs);CAND.write_text(json.dumps(refs,ensure_ascii=False,indent=2)+'\n');status=exc.response.status_code if isinstance(exc,requests.HTTPError) and exc.response is not None else None;error={'stage':'events','status':status,'reason':'rate_limited' if status==429 else type(exc).__name__};ref_events=len(unique_reference_events(refs));OBS.parent.mkdir(exist_ok=True);OBS.write_text('');SUMMARY.parent.mkdir(exist_ok=True);SUMMARY.write_text(json.dumps({'generated_at':now.isoformat(),'events_queried':0,'observations':0,'unique_markets':0,'top_markets':[],'top_leagues':[],'selection_fields':[],'provider_unavailable':True},indent=2)+'\n');UNIBET.write_text(json.dumps(observer_summary([],now),indent=2)+'\n');DIAG.write_text(json.dumps({'generated_at':now.isoformat(),'reference_rows':len(refs),'reference_events':ref_events,'exact_reference_events_in_bet365':0,'unmatched_reference_events':ref_events,'exact_reference_rows':0,'exact_rows_not_queried':0,'queried_reference_rows_without_h2h_price':0,'matched_prices':0,'bet365_events_available':0,'bet365_events_queried':0,'resolver_diagnostic_only':True,'resolver_accepted_events':0,'resolver_reason_counts':{},'resolver_matches':[],'resolver_rows_persisted':0,'resolver_rows_limit':MAX_RESOLVER_ROWS,'provider_unavailable':True},ensure_ascii=False,indent=2)+'\n');STATUS.write_text(json.dumps({'generated_at':now.isoformat(),'source':'odds-api.io','provider_unavailable':True,'bet365_events_available':0,'events_queried':0,'provider_call_attempts':1,'provider_call_successes':0,'odds_calls':0,'raw_market_observations':0,'unibet_observations':0,'unique_markets':0,'matched_reference_candidates':0,'resolver_accepted_events_diagnostic_only':0,'max_odds_calls':MAX_ODDS_CALLS,'errors':[error]},ensure_ascii=False,indent=2)+'\n');print('Bet365 provider unavailable for this run: '+json.dumps(error))
def main():
    ...
    unique_ref_keys=set(ref_keys);exact_ref_keys={k for k in unique_ref_keys if k in idx};resolver_rows,resolver_event_ids,resolver_counts,resolver_unique_events=resolver_diagnostics(refs,idx,events);prioritized=[];seen=set()
    for k in ref_keys:
        e=idx.get(k)
    ...
    prioritized=prioritized[:MAX_ODDS_CALLS];queried_ids={str(e.get('id')) for e in prioritized};cache,observations,unibet_rows,errors,attempts,successes,batch_attempts,batch_successes,fallback_attempts,fallback_successes=fetch_odds(prioritized,now);matched=0;exact_rows=0;exact_but_not_queried=0;queried_without_h2h_price=0
    for r in refs:
        home_name,away_name=ref_parts(r)
        if not home_name:continue
        e=idx.get(event_key(home_name,away_name))
        if not e:continue
        exact_rows+=1;r.update({'bet365_event_id':e.get('id'),'event_match_method':'exact'})
    ...
    CAND.write_text(json.dumps(refs,ensure_ascii=False,indent=2)+'\n');OBS.parent.mkdir(exist_ok=True);OBS.write_text(''.join(json.dumps(x,ensure_ascii=False,separators=(',',':'))+'\n' for x in observations));markets=collections.Counter(x['market'] for x in observations);leagues=collections.Counter(str(x.get('league') or 'unknown') for x in observations);selections=collections.Counter(x['selection'] for x in observations);raw_names=collections.Counter(str(x.get('raw_selection_name')) for x in observations if x.get('raw_selection_name') is not None);raw_labels=collections.Counter(str(x.get('raw_selection_label')) for x in observations if x.get('raw_selection_label') is not None);summary={'generated_at':now.isoformat(),'events_queried':len(prioritized),'observations':len(observations),'unique_markets':len(markets),'top_markets':markets.most_common(100),'top_leagues':leagues.most_common(50),'selection_fields':selections.most_common(50),'raw_selection_names':raw_names.most_common(100),'raw_selection_labels':raw_labels.most_common(100)};diagnostics={'generated_at':now.isoformat(),'reference_rows':len(refs),'reference_events':resolver_unique_events,'exact_reference_events_in_bet365':len(exact_ref_keys),'unmatched_reference_events':len(unique_ref_keys-exact_ref_keys),'exact_reference_rows':exact_rows,'exact_rows_not_queried':exact_but_not_queried,'queried_reference_rows_without_h2h_price':queried_without_h2h_price,'matched_prices':matched,'bet365_events_available':len(events),'bet365_events_queried':len(prioritized),'resolver_diagnostic_only':True,'resolver_accepted_events':len(resolver_event_ids),'resolver_reason_counts':dict(resolver_counts),'resolver_matches':resolver_rows,'resolver_rows_persisted':len(resolver_rows),'resolver_rows_limit':MAX_RESOLVER_ROWS};SUMMARY.parent.mkdir(exist_ok=True);SUMMARY.write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n');UNIBET.write_text(json.dumps(observer_summary(unibet_rows,now),ensure_ascii=False,indent=2)+'\n');DIAG.parent.mkdir(exist_ok=True);DIAG.write_text(json.dumps(diagnostics,ensure_ascii=False,indent=2)+'\n');STATUS.parent.mkdir(exist_ok=True);STATUS.write_text(json.dumps({'generated_at':now.isoformat(),'source':'odds-api.io','provider_unavailable':False,'bet365_events_available':len(events),'events_queried':len(prioritized),'provider_call_attempts':attempts,'provider_call_successes':successes,'multi_calls':batch_attempts,'multi_call_successes':batch_successes,'fallback_calls':fallback_attempts,'fallback_call_successes':fallback_successes,'raw_market_observations':len(observations),'unibet_observations':len(unibet_rows),'unique_markets':len(markets),'matched_reference_candidates':matched,'resolver_accepted_events_diagnostic_only':len(resolver_event_ids),'max_odds_calls':MAX_ODDS_CALLS,'errors':errors},ensure_ascii=False,indent=2)+'\n');print(json.dumps({'matched':matched,'observations':len(observations),'unibet_observations':len(unibet_rows),'unique_markets':len(markets),'exact_reference_rows':exact_rows,'calls':attempts,'successes':successes},ensure_ascii=False))
