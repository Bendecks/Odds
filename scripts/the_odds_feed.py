...
BOOKMAKERS=[x.strip() for x in os.getenv('THE_ODDS_BOOKMAKERS','pinnacle,betfair_ex_eu,unibet_eu,nordicbet,betsson').split(',') if x.strip()]
...
def main():
    ...
    now=datetime.now(timezone.utc); end=now+timedelta(hours=MAX_HOURS); candidates=[]; last_meta={}; events_seen=0
    ...
    for sport in SPORTS:
        try:data,last_meta=get(f'/v4/sports/{sport}/odds',params)
            ...
            if e.response is not None and e.response.status_code==422: continue
            raise
        ...
        for event in data:
            ...
            for pick,probs in consensus.items():
                ...
                # Reference-only candidate. A real-money PLAY still requires a separately verified Bet365 price.
    ...
    STATUS.write_text(json.dumps({'generated_at':now.isoformat(),'sports_requested':SPORTS,'bookmakers':BOOKMAKERS,'events_seen':events_seen,'candidates':len(candidates),'quota':last_meta},ensure_ascii=False,indent=2)+'\n')
    print(f'{len(candidates)} reference candidates from {events_seen} events; quota={last_meta}')
