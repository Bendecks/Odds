...
MODE='SMART_ENGINE_V1_MARKET_TYPES'
...
def is_soccer(k): return k.startswith('soccer_')
def market_type(sk,outcomes):
 names=[o.get('name','').lower() for o in outcomes]
 if 'draw' in names or 'uafgjort' in names:
  return '1X2_3WAY_REGULATION' if is_soccer(sk) else '3WAY_REGULATION_SKIP'
 if len(outcomes)==2:
  if sk.startswith('icehockey_'): return 'MONEYLINE_2WAY_INCL_OT_SO'
  if sk.startswith('basketball_'): return 'MONEYLINE_2WAY'
  if sk.startswith('tennis_'): return 'MATCH_WINNER_2WAY'
  return 'H2H_2WAY'
 return 'UNKNOWN_SKIP'
def novig(pr):
  ...
  for g in data:
   ...
   for b in books:
    for m in b.get('markets',[]):
     ...
     mt=market_type(sk,outcomes)
     if mt.endswith('_SKIP'): continue
     for o in outcomes:
      name=o.get('name','')
      if name.lower() in ['draw','uafgjort']: continue
      try: p=float(o['price'])
      except: continue
      by_type.setdefault(mt,{}).setdefault(name,[]).append(p)
   cands=[]
   for mt,mp in by_type.items():
    ...
    for sel,arr in mp.items():
     ...
     cands.append({'selection':sel,'odds':round(best,2),'market_type':mt,'edge_pct':round(edge*100,1),'value_pct':round(val*100,1),'confidence':round(conf,1),'stake_kr':stake(conf),'books':len(arr)})
   if cands:
    p=sorted(cands,key=lambda x:(x['confidence'],x['value_pct'],x['edge_pct'],x['books']),reverse=True)[0]
...
with open(OUT/'odds_feed.md','w',encoding='utf-8') as f:
 f.write('# SMART ENGINE V1 — MARKET TYPES FIXED\n\n'+summary+'\n\n')
