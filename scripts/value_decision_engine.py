...
POLICY_PATH=pathlib.Path('config/value_policy.json');DEFAULT={'mode':'PAPER','bankroll_dkk':50.0,'min_edge':0.02,'min_ev':0.025,'kelly_fraction':0.125,'max_stake_pct':0.03,'min_stake_dkk':1.0,'paper_ignore_min_stake':False,'bet365_min_stake_dkk':2.0,'max_price_age_minutes':20,'min_reference_books_for_play':3,'max_bets_per_event':1,'max_picks_per_run':25}
def policy():
...
def paper_mode():return str(P.get('mode','PAPER')).upper()=='PAPER'
def stake_gate_required():return not (paper_mode() and bool(P.get('paper_ignore_min_stake',False)))
def parse_dt(v):
 try:return datetime.fromisoformat(str(v).replace('Z','+00:00')).astimezone(timezone.utc)
 except Exception:return None
def minimum_odds(p):
 if p<=MIN_EDGE or KELLY_FRACTION<=0:return float('inf')
 gates=[(1+MIN_EV)/p,1/(p-MIN_EDGE)]
 if stake_gate_required():
  if BANKROLL*MAX_STAKE_PCT<MIN_STAKE:return float('inf')
...
def dnb_minimum_odds(pw,pl):
 ...
 q=pw/decisive;gates=[1+(MIN_EV+pl)/pw,1/(q-MIN_EDGE) if q>MIN_EDGE else float('inf')]
 if stake_gate_required():
  if BANKROLL*MAX_STAKE_PCT<MIN_STAKE:return float('inf')
  k=MIN_STAKE/(BANKROLL*KELLY_FRACTION)
  if q<=k:return float('inf')
  gates.append((1-k)/(q-k))
 return max(gates)
...
def evaluate(c,now=None):
 ...
 stake=min(BANKROLL*MAX_STAKE_PCT,BANKROLL*full*KELLY_FRACTION);stake=math.floor(stake*2)/2;enough_reference=books>=int(P.get('min_reference_books_for_play',3));stake_ok=stake>=MIN_STAKE or not stake_gate_required();qualified=enough_reference and edge>=MIN_EDGE and ev>=MIN_EV and stake_ok
 return {**c,'odds':odds,'implied_probability':round(implied,5),'edge':round(edge,5),'ev':round(ev,5),'stake':stake,'minimum_odds':round(mo,2) if math.isfinite(mo) else None,'enough_reference_for_play':enough_reference,'stake_gate_required':stake_gate_required(),'stake_eligible':stake>=MIN_STAKE,'bet365_min_stake_dkk':float(P.get('bet365_min_stake_dkk',2.0)),'qualified':qualified,'score':round(ev*max(edge,0),6)}
def decide(candidates,now=None):
 ...
 if not ranked:return {'decision':'NO BET','mode':mode,'bankroll':BANKROLL,'reason':'Ingen frisk verificeret Bet365-pris med eksakt event-identitet passerer reference-, edge- og EV-krav samt aktive indsatskrav.'}
