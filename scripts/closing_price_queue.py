...
def main():
    ...
    for x in signals:
        ...
        row={'signal_key':key,'event':x.get('event'),'event_id':x.get('event_id'),'sport':x.get('sport'),'market':x.get('market'),'line':x.get('line'),'pick':x.get('pick'),'taken_odds':x.get('odds'),'commence_time':x.get('commence_time'),'model_version':x.get('model_version'),'bet365_event_id':x.get('bet365_event_id'),'event_match_method':x.get('event_match_method')}
