...
def derive_for_event(rows):
    ...
    if not inputs:
        return []
    base, targets = inputs
    fit = fit_lambdas(targets)
    if not fit:
        return []
    mse, lh, la, p_yes = fit
    rmse = math.sqrt(mse)
    if rmse > MAX_RMSE or not (0 < p_yes < 1):
        return []
    p_no = 1.0 - p_yes
    books = min(int(r.get('books') or 0) for r in base)
    r = base[0]
    common = {
        'event': r.get('event'), 'event_id': r.get('event_id'), 'sport': r.get('sport'),
        'commence_time': r.get('commence_time'), 'market': 'btts', 'books': books,
        'reference_quality': 'strong' if books >= 4 else 'good', 'discovery_eligible': True,
        'bookmaker': 'DERIVED_REFERENCE_MARKET', 'bet365_verified': False,
        'model_version': 'market-consensus-v6-poisson-btts',
        'model_inputs': '1x2_consensus+totals_2.5_consensus',
        'poisson_home_lambda': round(lh, 3), 'poisson_away_lambda': round(la, 3),
        'model_fit_rmse': round(rmse, 6),
    }
    return [
        {**common, 'pick': 'yes', 'fair_probability': round(p_yes, 6), 'reference_odds': round(1/p_yes, 3)},
        {**common, 'pick': 'no', 'fair_probability': round(p_no, 6), 'reference_odds': round(1/p_no, 3)},
    ]
...
def main():
    ...
    for row in candidates:
        if row.get('market') not in ('double_chance', 'draw_no_bet', 'btts'):
            grouped.setdefault(str(row.get('event_id') or ''), []).append(row)
    derived = []
    for rows in grouped.values():
        derived.extend(derive_for_event(rows))
