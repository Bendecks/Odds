...
def fresh_bet365(candidate, now):
    ...
    if not stamp:
        return False
    max_age=float(engine.P.get('max_price_age_minutes',20))*60
    age=(now-stamp).total_seconds()
    return 0 <= age <= max_age

def candidate_edge(candidate):
    try:
        odds=float(candidate.get('bet365_odds',0))
        probability=float(candidate.get('fair_probability'))
    except Exception:
        return None, None
...
def bottleneck(funnel, decision):
    if decision.get('decision') in ('PAPER PICK','PLAY'):
        return 'Der er en kvalificeret modelbeslutning.'
    checks=[
        ('candidate_rows','Ingen referencekandidater i den seneste kørsel.'),
...
def main():
    ...
    bet365=load_json(BET365,{})
    diagnostics=load_json(REFERENCE_DIAG,{})
    market=load_json(MARKET_COVERAGE,{})
    runs=read_jsonl(DECISION_RUNS)
    ...
    funnel={
        ...
        'fair_probability_rows':sum(1 for c in candidates if c.get('fair_probability') is not None),
        'reference_events':diagnostics.get('reference_events'),
        'exact_bet365_rows':len(exact),
        'fresh_exact_bet365_rows':len(fresh),
        'reference_depth_ready_rows':len(reference_ready),
        'positive_edge_rows':len(positive_edge),
        'ev_ready_rows':len(ev_ready),
        'qualified_now_rows':len(qualified),
        'decision_runs_recorded':len(runs)
    }
    provider={
        'generated_at':bet365.get('generated_at'),
        'available':provider_available(bet365),
        'bet365_events_available':bet365.get('bet365_events_available'),
        'events_queried':first_present(bet365,'events_queried','queried_events'),
        'provider_call_attempts':bet365.get('provider_call_attempts'),
        'provider_call_successes':bet365.get('provider_call_successes'),
        'odds_calls':bet365.get('odds_calls'),
        'odds_multi_calls':first_present(bet365,'odds_multi_calls','batch_attempts'),
        'fallback_odds_calls':first_present(bet365,'fallback_odds_calls','fallback_attempts'),
        'raw_market_observations':bet365.get('raw_market_observations'),
        'unique_markets':bet365.get('unique_markets') or market.get('unique_markets'),
        'matched_reference_candidates':bet365.get('matched_reference_candidates'),
        'errors':bet365.get('errors') or []
    }
    report={
        'generated_at':now.isoformat(),
        'mode':decision.get('mode') or engine.P.get('mode','PAPER'),
        'decision':decision.get('decision'),
        'reason':decision.get('reason'),
        'bottleneck':bottleneck(funnel,decision),
        'funnel':funnel,
        'provider':provider,
        'policy':{
            'bankroll_dkk':engine.P.get('bankroll_dkk'),
            'min_edge':engine.P.get('min_edge'),
            'min_ev':engine.P.get('min_ev'),
            'max_price_age_minutes':engine.P.get('max_price_age_minutes'),
            'min_reference_books_for_play':engine.P.get('min_reference_books_for_play')
        }
    }
