...
def edge_diagnostics(rows):
    ...
    top=[]
    for edge,ev,c in ranked[:10]:
        top.append({'event':c.get('event'),'market':c.get('market'),'pick':c.get('pick'),'line':c.get('line'),'books':c.get('books'),'bet365_odds':c.get('bet365_odds'),'fair_probability':c.get('fair_probability'),'edge':round(edge,6),'ev':round(ev,6) if ev is not None else None})
...
def main():
    now=datetime.now(timezone.utc);candidates=load_json(CANDIDATES,[]);decision=load_json(DECISION,{});bet365=load_json(BET365,{});diagnostics=load_json(REFERENCE_DIAG,{});market=load_json(MARKET_COVERAGE,{});runs=read_jsonl(DECISION_RUNS)
    ...
    diagnostics_out={'by_market':{'candidate':market_counts(candidates),'exact':market_counts(exact),'fresh':market_counts(fresh),'reference_ready':market_counts(reference_ready),'production_edge_ready':market_counts(positive_edge)},'exact_price_age_buckets':age_buckets(exact,now),'reference_depth_rejections_by_market':market_counts([c for c in fresh if int(c.get('books') or 0)<min_books]),'edge_distribution':edge_diagnostics(reference_ready)}
