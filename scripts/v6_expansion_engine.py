...
ODDS_IO_MAX_EVENTS = int(os.getenv('ODDS_API_IO_MAX_EVENTS', '10'))
ODDS_IO_SPORTS = os.getenv('ODDS_IO_SPORTS', 'football,basketball,ice-hockey,tennis').split(',')
THEODDS_SPORTS = os.getenv('THEODDS_SPORTS', 'soccer_epl,soccer_spain_la_liga,soccer_germany_bundesliga,basketball_nba,icehockey_nhl').split(',')
MARKETS = ('h2h', 'totals')
MODE = 'V13_NEAR_TERM_STRICT_READY_ENGINE'
...
def sport_bucket(s):
    s = str(s or '').lower()
    if s.startswith('soccer_') or s in ('football', 'soccer_odds_api_io'): return 'soccer'
        ...
        if isinstance(data, list):
            for g in data:
                if isinstance(g, dict) and upcoming(g.get('commence_time')):
                    g['source'] = 'the-odds-api'; games.append(g)
...
def odds_io_pick_leagues(sport):
    preferred = {'football': ['england-premier-league', 'spain-laliga', 'germany-bundesliga'], 'basketball': ['usa-nba'], 'ice-hockey': ['usa-nhl'], 'tennis': []}.get(sport, [])
        ...
        for m in rows if isinstance(rows, list) else []:
            ...
            if name in ('ml', 'h2h', 'moneyline'):
                if odds_rows and isinstance(odds_rows[0], dict) and any(k in odds_rows[0] for k in ('home', 'away', 'draw')):
                    ...
                    if row.get('draw') is not None: outs.append({'name': 'draw', 'price': row.get('draw')})
                else:
                    for row in odds_rows:
                        if isinstance(row, dict):
                            nm = row.get('name') or row.get('label'); pr = row.get('price') or row.get('odds')
                            if nm and str(nm).lower() != 'draw': outs.append({'name': nm, 'price': pr})
                if outs: markets.append({'key': 'h2h', 'outcomes': outs})
            elif name == 'totals':
                for row in odds_rows:
                    if not isinstance(row, dict): continue
                    if row.get('over') is not None: outs.append({'name': 'Over', 'price': row.get('over'), 'point': row.get('hdp') or row.get('point')})
                    if row.get('under') is not None: outs.append({'name': 'Under', 'price': row.get('under'), 'point': row.get('hdp') or row.get('point')})
    ...
    for sport in [s.strip() for s in ODDS_IO_SPORTS if s.strip()]:
        for league in odds_io_pick_leagues(sport):
            ...
            for ev in near_events[:ODDS_IO_MAX_EVENTS]:
                ...
                odds = first_dict(raw_odds)
                home = odds.get('home') or ev.get('home'); away = odds.get('away') or ev.get('away')
...
def add_candidate(cands, g, market, selection, prices, point=None):
    ...
    if not vals: return
    best = max(vals); med = statistics.median(vals); books = len(vals); single_source = books < 2
    edge = 0.0 if single_source else (best / med - 1 if med else 0.0)
    if single_source:
        score = 6.2 if market == 'h2h' and 1.45 <= best <= 3.4 else 5.7 if market == 'totals' and 1.55 <= best <= 2.4 else 4.0
    ...
    if best >= 4: score -= 2
    if score < 5.5: return
    cands.append({'event': f"{g.get('home_team')} vs {g.get('away_team')}", 'event_id': g.get('id'), 'source': g.get('source') or 'unknown', 'sport': g.get('sport_key'), 'sport_bucket': sport_bucket(g.get('sport_key')), 'start': g.get('commence_time'), 'start_local': fmt_start(g.get('commence_time')), 'market': market, 'selection': selection, 'point': point, 'odds': round(best, 2), 'median': round(med, 2), 'edge_pct': round(edge * 100, 1), 'books': books, 'pre_score': round(score, 2), 'confidence': confidence, 'single_source': single_source})
...
def rank(cands):
    ...
    for c in cands:
        ...
        if not top_eligible(c): item['role'] = 'WATCHLIST'; item['stake_kr'] = 0; item['reason'] = 'Ikke stærk nok til top bet.'; watch.append(item); continue
        if any(conflicts(c, t) for t in top): item['role'] = 'WATCHLIST'; item['stake_kr'] = 0; item['reason'] = 'Konflikt: allerede valgt andet spil i samme kamp.'; watch.append(item); continue
        item['role'] = 'PRIMARY'; item['stake_kr'] = unit_stake(c); item['reason'] = 'Near-term engine pick. Strict Mode filtrerer efterfølgende.'
        top.append(item)
