...
THEODDS_SPORTS = os.getenv('THEODDS_SPORTS','soccer_epl,soccer_spain_la_liga,soccer_germany_bundesliga,basketball_nba,tennis_atp,tennis_wta,icehockey_nhl,baseball_mlb,americanfootball_nfl,mma_mixed_martial_arts').split(',')
ODDS_IO_BASE = 'https://api.odds-api.io/v3'
ODDS_IO_BOOKMAKER = os.getenv('ODDS_API_IO_BOOKMAKER','1xbet')
ODDS_IO_SPORTS = os.getenv('ODDS_IO_SPORTS','football,tennis,basketball,ice-hockey').split(',')
ODDS_IO_MAX_EVENTS = int(os.getenv('ODDS_API_IO_MAX_EVENTS','8'))
DIAG = {'api_errors':[],'quota_exhausted':False,'cache_used':False,'cache_written':False,'odds_api_io_used':False,'odds_api_io_raw_games':0,'odds_api_io_events':0,'odds_api_io_odds_calls':0,'theodds_sports_used':0,'theodds_sport_games':0,'unique_games':0,'games_after_filter':0,'candidate_count_before_sort':0,'top_eligible_count':0}
...
def mark_error(label,e):
    s = str(e)[:700]
    if 'OUT_OF_USAGE_CREDITS' in s or 'Usage quota' in s or 'exceeded your rate limit' in s:
...
def safe_get(label, url, params=None):
    if DIAG['quota_exhausted'] and label.startswith('theodds'):
        return []
    try:
...
def odds_io_pick_league(leagues):
    ...
    preferred = ['england-premier-league','spain-laliga','italy-serie-a','germany-bundesliga','france-ligue-1','denmark-superliga','usa-nba','usa-nhl','atp-atp-rome-italy-men-singles','wta-wta-rome-italy-women-singles']
...
def fetch_odds_api_io_sport(sport):
    ...
    for ev in events[:ODDS_IO_MAX_EVENTS]:
        ...
        if g: games.append(g)
    if games:
        DIAG['odds_api_io_used']=True
        DIAG['odds_api_io_raw_games'] += len(games)
    return games
...
def collect_candidates():
    ...
    if ODDS:
        ...
        DIAG['theodds_sports_used']=len(THEODDS_SPORTS); DIAG['theodds_sport_games']=len(all_games)
    ...
    if not all_games:
        ...
        if all_games: save_cache(all_games)
...
def gemini_rank(cands,conflict_watch):
    shortlist=[c for c in cands if is_top_eligible(c)][:80]
    watch_seed=[c for c in cands if not is_top_eligible(c)][:80]+conflict_watch[:80]
    if not GEMINI or not shortlist: return {'summary':'fallback ranking','top_bets':fallback_rank(cands),'watchlist':watch_seed[:80],'pass':[]}
    prompt='''Du er Bendix V7 Multi-Sport Top Bet Governor. Singles only, ingen livebetting, ingen parlays. Maks 20 top_bets, maks 1 pr event. Returner dansk JSON only: summary, top_bets, watchlist, pass. Hvert item: event, market, pick, point, odds, confidence, role, reason. Data:\n'''+json.dumps({'shortlist':shortlist,'watch_seed':watch_seed[:80]},ensure_ascii=False)
...
res['top_bet_governor']={'max_top_bets':MAX_TOP_BETS,'min_top_score':MIN_TOP_SCORE,'max_auto_odds':MAX_AUTO_ODDS,'odds_api_io_bookmaker':ODDS_IO_BOOKMAKER,'display_tz':DISPLAY_TZ}
