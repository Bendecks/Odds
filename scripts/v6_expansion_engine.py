...
ODDS_IO_MAX_EVENTS = int(os.getenv('ODDS_API_IO_MAX_EVENTS', '6'))
...
MARKETS = ('h2h', 'totals')
MODE = 'V12_SAFE_STRICT_READY_ENGINE'
...
DIAG = {
    ...
    'candidate_count': 0,
    'top_count': 0,
    'engine_note': 'Safe engine: handles dict/list odds-api.io responses and avoids crash on malformed API payloads.'
}
...
def sport_bucket(s):
    s = str(s or '').lower()
    if s.startswith('soccer_') or s in ('football', 'soccer_odds_api_io'):
        return 'soccer'
    if s.startswith('basketball_') or s == 'basketball':
        return 'basketball'
    if s.startswith('icehockey_') or s == 'ice-hockey':
        return 'icehockey'
    if s.startswith('tennis_') or s == 'tennis':
        return 'tennis'
    return 'other'
...
def mark_error(label, err):
    txt = str(err)[:700]
    if 'OUT_OF_USAGE_CREDITS' in txt or 'Usage quota' in txt:
...
def odds_io_pick_leagues(sport):
    preferred = {
        'football': ['england-premier-league', 'spain-laliga', 'germany-bundesliga'],
        'basketball': ['usa-nba'],
        'ice-hockey': ['usa-nhl'],
        'tennis': []
    }.get(sport, [])
...
def odds_io_markets(raw_odds, home, away):
    ...
    if isinstance(bobj, list):
        # Some responses return [{name/key, markets:[...]}]. Convert to dict-like rows.
        converted = {}
        for row in bobj:
            ...
            if name in ('ml', 'h2h', 'moneyline'):
                if odds_rows and isinstance(odds_rows[0], dict) and any(k in odds_rows[0] for k in ('home', 'away', 'draw')):
                    ...
                    if row.get('draw') is not None:
                        outs.append({'name': 'draw', 'price': row.get('draw')})
                else:
                ...
                for row in odds_rows:
                    ...
                    if row.get('over') is not None:
                        outs.append({'name': 'Over', 'price': row.get('over'), 'point': row.get('hdp') or row.get('point')})
                    if row.get('under') is not None:
                        outs.append({'name': 'Under', 'price': row.get('under'), 'point': row.get('hdp') or row.get('point')})
                if outs:
...
def add_candidate(cands, g, market, selection, prices, point=None):
    ...
    cands.append({
        ...
        'sport_bucket': sport_bucket(g.get('sport_key')),
...
def top_eligible(c):
    ...
    if odds > 6:
        return False
    if c.get('single_source') and (score < 5.7 or odds > 3.6):
        return False
    if not c.get('single_source') and score < 7:
        return False
    return True
...
def conflicts(a, b):
    if a.get('event') != b.get('event'):
        return False
    if a.get('market') == 'h2h' and b.get('market') == 'h2h':
        return True
    if a.get('market') == b.get('market') and str(a.get('point')) == str(b.get('point')):
        return True
    return False
...
def rank(cands):
    ...
    for c in cands:
        ...
        item['role'] = 'PRIMARY'
        item['stake_kr'] = unit_stake(c)
        item['reason'] = 'Safe engine pick. Strict Mode filtrerer efterfølgende.'
        top.append(item)
        if len(top) >= MAX_TOP_BETS:
            break
